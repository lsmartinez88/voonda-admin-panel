...
class ApiEnrichmentService {
    ...
    static async enrichWithOpenAI(matchingResults, options = {}, onProgress = null) {
        try {
            ...
            const openaiResults = await OpenAIService.processVehiclesBatch(vehicleQueries, {
                batchSize,
                delayBetweenBatches,
                onProgress: (progress) => {
            ...
            const enrichedResults = matchingResults.map((result) => {
                ...
                const openaiResult = openaiResults.results.find((openaiRes) => openaiRes.vehicleQuery?.originalResult === result)
    ...
    static async enrichComplete(matchingResults, options = {}, onProgress = null) {
        try {
            ...
            const { enableCatalogEnrichment = true, enableOpenAI = true, ...openaiOptions } = options
            ...
            if (enableCatalogEnrichment) {
                ...
                const catalogEnrichment = await this.enrichMatchingResults(currentResults, (progress) => onProgress && onProgress({ stage: "catalog", ...progress }))
            ...
            if (enableOpenAI) {
                ...
                const openaiEnrichment = await this.enrichWithOpenAI(currentResults, openaiOptions, (progress) => onProgress && onProgress({ stage: "openai", ...progress }))
